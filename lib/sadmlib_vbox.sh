...
export VBOXMANAGE=$(which VBoxManage)                                   # Usually /usr/bin/VBoxManage
export CUR_DATE=$(date "+%C%y_%m_%d")                                   # Date Format 2018_05_27

# Variables already loaded by sadmlib_std.sh.
# You change them in $sadmin/cfg/sadmin.cfg).
...
# SADMIN VM Tools can only run on a system that have the $VMHOST_CFG created (Empty file).
    ...
    if [ -f "$VMHOST_CFG" ]                                             # Server File exist = VM Use
        ...
        else sadm_write_err "[ ERROR ] This system is not a valid Virtual Box for SADMIN 'vmtools'." 

...
function display_data($result) {    
    ...
    $URL_VIEW_FILE  = '/view/log/sadm_view_file.php';                   # Page to view log received
    $URL_VIEW_RCH   = '/view/rch/sadm_view_rchfile.php';                # Page to View RCH received
    $URL_OSUPDATE   = '/crud/srv/sadm_server_osupdate.php';             # Page to Update O/S Update
    ...
    while ($row = mysqli_fetch_assoc($result)) {                        # Loop Result from Query
        ...
        $scriptName= pathinfo(SADM_OSUPDATE_SCRIPT,PATHINFO_FILENAME);  # Script without Extension
        ...
        echo "\n<td align='left'>";
        echo "<a href='" . $URL_HOST_INFO . "?sel=" . $row['srv_name'] . "&back=" . $URL_VIEW_SCHED;
        echo "' title='" . $row['srv_osname'] . " " . $row['srv_osversion'] . " system, click for more info'>";
        echo $row['srv_name'] ."<br></a>" . $row['srv_desc'];
        echo "</td>";
        ...
        if ($WLAST_UPDATE != "None") {
            ...
            if ($osupdate_age > SADM_OSUPDATE_INTERVAL) {                               # If was more than threshold
                $tooltip = "Last O/S update was done " .$osupdate_age. " days ago, threshold at " . SADM_OSUPDATE_INTERVAL. " days.";
        ...
        if ($row['srv_update_auto'] == True) {                          # Is Automatic update on
            $tooltip = 'Schedule is active, click to modify schedule.';
        }else{                                                          # If Shedule Inactive
...
# ==================================================================================================
    ...
    if ($DEBUG) { echo "<br>1st Parameter Received is '" . $SELECTION . "'"; } 
    ...
    switch ($SELECTION) {
        case 'all_servers'  : 
            ...
            #$sql = "SELECT * FROM server where srv_ostype = 'linux' order by srv_name;";
    ...
    # Diplay Page Heading
    display_lib_heading("NotHome","$TITLE"," ",$Page_Version);          # Display Content Heading
    ...
    display_data($result);                                              # Display array of result

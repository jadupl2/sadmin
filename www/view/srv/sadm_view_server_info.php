...
# 2018_10_28 v2.6 Add prefix 'cfg2html_' to filename generated by cfg2html & link to files.
...
function display_right_side ($wrow) {
    ...
    # Date in the month the backup will run
    $BDATE = "Run ";                                                    # Result Date Backup Run
    if (substr($wrow['srv_backup_dom'],0,1) == "Y") {                   # If DOM begin with Y=AnyDate
        $BDATE = "";                                                    # Backup run every date
    }else{                                                              # If not Get date it Run
        for ($i = 1; $i < 32; $i = $i + 1) {
            if ((substr($wrow['srv_backup_dom'],$i,1) == "Y") && ($i == 1)) { $BDATE = $BDATE . "1st," ;}
            if ((substr($wrow['srv_backup_dom'],$i,1) == "Y") && ($i == 2)) { $BDATE = $BDATE . "2nd," ;}
            if ((substr($wrow['srv_backup_dom'],$i,1) == "Y") && ($i == 3)) { $BDATE = $BDATE . "3rd," ;}
            if ((substr($wrow['srv_backup_dom'],$i,1) == "Y") && ($i > 3))  { $BDATE = $BDATE . $i . "th," ;}
        }
    }
    echo rtrim($BDATE, ',') ;                                           # Remove Trailing Comma
    if (substr($wrow['srv_backup_month'],0,1) != "Y") { echo " of " ; } # If Specific Mth print "of"

    # Month that the Backup will Run
    $mth_name = array('every month, ','Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec');
    $BMTH = "";
    if (substr($wrow['srv_backup_month'],0,1) == "Y") {                 # If Mth begin with Y=AnyMTH
        $BMTH = $mth_name[0];                                           # Backup run every Month
        if (substr($wrow['srv_backup_dom'],0,1) == "Y") { $BMTH = ""; }
    }else{                                                              # If not Get date it Run
        for ($i = 1; $i < 13; $i = $i + 1) {
            if (substr($wrow['srv_backup_month'],$i,1) == "Y") { $BMTH = $BMTH . $mth_name[$i] ; }
        }
    }
    echo rtrim($BMTH, ',') . " " ;                                      # Remove Trailing Comma

    # Days of the week the backup will run
    $BDAYS = "";                                                        # Result Day(s) Backup run
    if (substr($wrow['srv_backup_dow'],0,1) == "Y") {                   # If DOW begin with Y=AllWeek
        $BDAYS = "All week ";                                           # Backup run every day
    }else{                                                              # If not Get Days it Run
        if (substr($wrow['srv_backup_dow'],1,1) == "Y") { $BDAYS = $BDAYS . "Sunday,"    ;}
        if (substr($wrow['srv_backup_dow'],2,1) == "Y") { $BDAYS = $BDAYS . "Monday,"    ;}
        if (substr($wrow['srv_backup_dow'],3,1) == "Y") { $BDAYS = $BDAYS . "Tuesday,"   ;}
        if (substr($wrow['srv_backup_dow'],4,1) == "Y") { $BDAYS = $BDAYS . "Wednesday," ;}
        if (substr($wrow['srv_backup_dow'],5,1) == "Y") { $BDAYS = $BDAYS . "Thursday,"  ;}
        if (substr($wrow['srv_backup_dow'],6,1) == "Y") { $BDAYS = $BDAYS . "Friday,"    ;}
        if (substr($wrow['srv_backup_dow'],7,1) == "Y") { $BDAYS = $BDAYS . "Saturday,"  ;}
    }
    echo rtrim($BDAYS, ',');                                            # Remove Trailing Comma

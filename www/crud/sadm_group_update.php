...
# ==================================================================================================
    ...
    if (isset($_POST['submitted'])) {
        ...
        # If Default Group is True, Check if there is already a default Group, Can't have two
        if ($_POST['scr_default']) {
            $sql = "SELECT grp_code, grp_default FROM sadm.group ";  # Construct SQL Statement
        ...
        $sql = $sql . "grp_status = '"      . sadm_clean_data($_POST['scr_status'])     ."'  ";
        ...
        if (!$row){                                                     # If Update didn't work
            ...
            sadm_alert ("Group '".$_POST['scr_code']."' updated.");  # Advise user of success
        }
...
# ==================================================================================================
    ...
    if (isset($_GET['sel'])) {                                          # If row selection Receive
        ...
        $query = "SELECT * FROM sadm.group WHERE grp_code = '" . $wkey . "' ;";  # Construct SQL
        ...
        if (!$result) {                                                 # If row wasn't found
            $err_msg = "ERROR : Group wasn't found in Database\n";   # Row was not found Msg.
        ...
        sadm_alert ($err_msg) ;                                         # Display Error Msg. Box
        ?><script>location.replace("/crud/sadm_group_main.php");</script><?php # Back 2 List Page
        exit ;
    }
    $title = "Update a Group" ;                                      # Page Heading Title

...
BACK_SERVER="ds409.maison.ca"                       ; export BACK_SERVER    # Remoote Backup Name
...
# Need to received Remote Input Directory and Local Output Directory
    ...
    CMD="rsync -e 'ssh -p 22' -vrOltD --delete  --exclude 'lost+found'  ${IDIR}/ ${BACK_SERVER}:${ODIR}/"
    
    write_log "$CMD"

    rsync -e 'ssh -p 22' -vrOltD --delete  --exclude 'lost+found'  ${IDIR}/ ${BACK_SERVER}:${ODIR}/ >>$LOG_FILE 2>&1
    #$CMD  >>$LOG_FILE 2>&1
    RC=$?
    if [ $RC -ne 0 ]
         then    write_log "RESULT : rsync $IDIR to $ODIR - ERROR $RC"
                 RC=1
         else    write_log "RESULT : rsync $IDIR to $ODIR - OK"
                 RC=0
    fi
...
#
    ...
    if [ $RC -eq 0 ] 
       then tail -2 $LOG_STAT | mail -s "SADM - Success of $INST on $HOSTNAME"   $SYSADMIN
       else cat $LOG_FILE     | mail -s "SADM - Problem with $INST on $HOSTNAME" $SYSADMIN
    fi

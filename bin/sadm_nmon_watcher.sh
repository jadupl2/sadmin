...
#   Synopsis : Start 'nmon' performance daemon, if it's not already running (End Day at 23:58:58)
#   Version  : 1.5
#   Date     : November 2015 
#   Requires : sh
# --------------------------------------------------------------------------------------------------
# Change log
#
# 2016_10_10 sysmon  v1.7 Modification making sure that nmon is available on the server.
#            sysmon  If not found a copy in /sadmin/pkg/nmon/aix in used to create the link /usr/bin/nmon
# 2016_11_11 sysmon  v1.8 Enhance checking for nmon existence and add some message for user, ReTested AIX
# 2016_12_20 sysmon  v1.9 Change to run on Aix 7.x and minor corrections
# 2017_12_29 sysmon  v2.0 Add Warning message stating that nmon not available on MacOS
# 2017_01_27 sysmon  V2.1 Now list two newest nmon files in $SADMIN/dat/nmon & Fix minor Bug & add comment
# 2017_02_02 sysmon  V2.2 Show number of nmon running only once, if nmon is already running
# 2017_02_04 sysmon  V2.3 Snapshot will be taken every 2 minutes instead of 5.
# 2017_02_08 sysmon  V2.4 Fix Compatibility problem with 'sadh' shell (If statement) 
...
check_nmon()
{
    ...
    if [ $nmon_count -gt 1 ] 
        then sadm_write_log "Found another nmon process running with s120 parameter"

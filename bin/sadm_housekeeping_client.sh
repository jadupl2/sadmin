...
file_housekeeping()
{
    ...
    if [ -d "$SADM_TMP_DIR" ]
        then sadm_logger "find $SADM_TMP_DIR -type f -mtime +7 -exec rm -f {} \;"
             find $SADM_TMP_DIR  -type f -mtime +7 -exec ls -l {} \; | tee -a $LOG
    ...
    if [ -d "${SADM_RCH_DIR}" ]
        then sadm_logger "Find any *.rch file older than ${LIMIT_DAYS} days in ${SADM_RCH_DIR} and delete them"
             find ${SADM_RCH_DIR} -type f -mtime +${LIMIT_DAYS} -name "*.rch" -exec ls -l {} \; | tee -a $LOG
             find ${SADM_RCH_DIR} -type f -mtime +${LIMIT_DAYS} -name "*.rch" -exec rm -f {} \; | tee -a $LOG
             if [ $? -ne 0 ]
                then sadm_logger "Error occured on the last operation."
                     ERROR_COUNT=$(($ERROR_COUNT+1))
                else sadm_logger "OK"
    ...
    if [ -d "${SADM_LOG_DIR}" ]
        then sadm_logger "Find any *.log file older than ${LIMIT_DAYS} days in ${SADM_LOG_DIR} and delete them"
             find ${SADM_LOG_DIR} -type f -mtime +${LIMIT_DAYS} -name "*.log" -exec ls -l {} \; | tee -a $LOG
             find ${SADM_LOG_DIR} -type f -mtime +${LIMIT_DAYS} -name "*.log" -exec rm -f {} \; | tee -a $LOG
             if [ $? -ne 0 ]
                then sadm_logger "Error occured on the last operation."
                     ERROR_COUNT=$(($ERROR_COUNT+1))
                else sadm_logger "OK"

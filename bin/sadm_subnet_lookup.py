...
pdebug      = 0                                                      # Debug level from 0 to 9
pexit_code  = 0                                                      # Script default exit code
db_conn     = ''                                                     # Database Connector (if used)
db_cur      = ''                                                     # Database Cursor (if used)
...
def db_update(wip,db_conn,db_cur,wzero,wname,wmac,wman,wping,wdateping,wdatechange):
    ...
    if pdebug > 4 : print ("db_update dbconn,db_cur : ", db_conn, db_cur)
...
def scan_network(snet,db_conn,db_cur) :
    ...
    if pdebug > 4 : print ("scan_network dbconn,db_cur : ", db_conn, db_cur)
...
def main_process(db_conn,db_cur):
    ...
    if pdebug > 4 : 
        print ("\nmain_process dbconn,db_cur : ", db_conn, db_cur)
...
def main(argv):
    ...
    if pdebug > 4 : print ("\nmain dbconn,db_cur : ", db_conn, db_cur)
    pexit_code = main_process(db_conn,db_cur)                           # Loop All Active systems
    if pdebug > 4 :print ("\nmain_after dbconn,db_cur : ", db_conn, db_cur)

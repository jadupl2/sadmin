...
help()
{
    ...
    echo "             -e   (Enable SADM Startup Service"
    echo "             -d   (Disable SADM Startup Service"
...
service_disable()
{
    ...
    if [ $SYSTEMD -eq 1 ]                                               # Using systemd not Sysinit
        then sadm_writelog "systemctl disable $1"                       # write cmd to log
             systemctl disable $1 >> $SADM_LOG 2>&1                     # use systemctl to disable
...
service_enable()
{
    ...
    if [ $SYSTEMD -eq 1 ]                                               # Using systemd not Sysinit
        then sadm_writelog "cp $SADM_SRV_IFILE $SADM_SRV_OFILE"         # write cmd to log
             ...
             sadm_writelog "systemctl enable $1"                        # write cmd to log
             systemctl enable $1    >> $SADM_LOG 2>&1                   # use systemctl
...
service_status()
{
    ...
    if [ $SYSTEMD -eq 1 ]                                               # Using systemd not Sysinit
        then sadm_writelog "systemctl status $1"                        # write cmd to log
             systemctl status $1 | tee -a $SADM_LOG 2>&1                # use systemctl
             systemctl list-unit-files | grep $1 >> $SADM_LOG 2>&1      # Display Service Status
...
service_stop()
{
    ...
    if [ $SYSTEMD -eq 1 ]                                               # Using systemd not Sysinit
        then sadm_writelog "systemctl stop $1"                          # write cmd to log
             systemctl stop $1    >> $SADM_LOG 2>&1                     # use systemctl
...
service_start()
{
    ...
    if [ $SYSTEMD -eq 1 ]                                               # Using systemd not Sysinit
        then sadm_writelog "systemctl start $1"                         # write cmd to log
             systemctl start $1    >> $SADM_LOG 2>&1                    # use systemctl
...
# --------------------------------------------------------------------------------------------------
    ...
    while getopts "deh " opt ; do                                       # Loop to process Switch
        case $opt in
            d) P_DISABLE="ON"                                           # Disable SADM Service 
               service_stop    sadmin.service                           # Stop sadmin Service
               service_disable sadmin.service                           # Enable sadmin Service
               service_status  sadmin.service                           # Status of sadmin Service
               sadm_stop 0                                              # Close the shop
               exit 0                                                   # Back to shell 
               ;;                                                      
            e) P_ENABLE="ON"                                            # Enable SADM Service 
               service_enable sadmin.service                            # Enable sadmin Service
               service_start  sadmin.service                            # Start sadmin Service
               service_status  sadmin.service                           # Status of sadmin Service
               sadm_stop 0                                              # Close the shop
               exit 0                                                   # Back to shell 
               ;;                                                      

...
initialization()
{
    ...
    write_log "Mounting the source NFS Drive on $FROM_SERVER"
    write_log "mount ${FROM_SERVER}:${FROM_BASE_DIR} ${LOCAL_DIR1}"
    umount ${LOCAL_DIR1} > /dev/null 2>&1
    mount ${FROM_SERVER}:${FROM_BASE_DIR} ${LOCAL_DIR1} >>$LOG_FILE 2>&1
    RC=$?
    if [ "$RC" -ne 0 ]
        then RC=1
             write_log "Mount NFS Failed from $FROM_SERVER Proces Aborted"
             return 1
    fi
    
    # Mount the TO NFS Drive (Destination)
    write_log "Mounting the destination NFS Drive on $TO_SERVER"
    write_log "mount ${TO_SERVER}:${TO_BASE_DIR} ${LOCAL_DIR2}"
    umount ${LOCAL_DIR2} > /dev/null 2>&1
    mount ${TO_SERVER}:${TO_BASE_DIR} ${LOCAL_DIR2} >>$LOG_FILE 2>&1
    RC=$?
    if [ "$RC" -ne 0 ]
        then RC=1
             write_log "Mount NFS Failed from $TO_SERVER Proces Aborted"
             return 1
    fi
...
{
   RC=$1
   write_log "Unmounting NFS mount directories"
   write_log "umount ${LOCAL_DIR1}"
   umount ${LOCAL_DIR1} >> $LOG_FILE 2>&1
   write_log "umount ${LOCAL_DIR2}"   
...
# Do the Rsync
    write_log "rsync -var ${LOCAL_DIR1}/ ${LOCAL_DIR2}/"
    rsync -var ${LOCAL_DIR1}/ ${LOCAL_DIR2}/ >>$LOG_FILE 2>&1
    RC=$? 
    if [ $RC -ne 0 ]
       then write_log "ERROR NUMBER $RC on rsync "
            RC=1
    fi

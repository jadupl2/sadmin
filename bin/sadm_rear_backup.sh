...
export REAR_CUR_ISO="${REAR_NAME}.iso"     
export REAR_CUR_BAC="${REAR_NAME}.tar.gz"                               # Rear Host Backup File Name
export REAR_CUR_BAC_LOG="${REAR_NAME}.tar.gz"                           # Rear Host Backup File Name
...
rear_preparation()
{
    ...
    sadm_write_log " "
    sadm_write_log " "
    sadm_write_log "Content of ReaR site file ${REAR_CFGFILE}."
    grep -Ev "^$|^#" $REAR_CFGFILE | while read wline ; do sadm_write_log "${wline}"; done
    sadm_write_log " "
    
    sadm_writelog "Before backup - List of ReaR backup and ISO actually on NFS Server for ${SADM_HOSTNAME}"
    ls -ltrh ${NFS_MOUNT}/${SADM_HOSTNAME} | while read wline ; do sadm_write "${wline}\n"; done

#!/usr/bin/perl
# --------------------------------------------------------------------------------------------------
# Standard Life Application Monitor (SLAM) - Rename SCOM_slave in Nov 2010
# Written in August 1998 by Jacques Duplessis.
# SLAMDIR Environment Variable must be define and must indicate
# the location of the slam.cfg file (export SLAMDIR=/sysadmin/slamv2)
# This program read the `hostname`.cfg file located in $SLAMDIR and update it every time is run.
#
# - Version 4 - September 2002
#   - Add Load average monitoring
#   - Add Minute delay before trigger
#   - Add Qpage Aliases Definition when error occur
#   - Add Email Aliases Definition when error occur
#   - Add Possibility to Run script when error occur
#   - Add Monitoring for Hitachi Path Manager
#   - Run Script for specific need                
# 04.00e  - Replace ps -el by ps -ef in daemon routine
# 04.00f  - When Checking for a daemon used ps -ef AND ps -el and use the largest number
# 04.00g  - Change Errpt handling - If Temporary Error = Warning - If permanent = Beeper
# 04.00h  - Added TSM Path Verification
# 04.00i  - Added Check SLAMRCP_FILE and SLAMSCP_FILE to use scp or rcp file transfer
# 04.00j  - Correction for HDS DataPath Query Adapter value return get rid of the - 
# 04.00h  - If errpt report a problem with tape (rmt) change Permanent to Temporary Error
# 04.00i  - check ccinet status between jobtrack and Autosys server V4
# 04.00j  - Add Support for DB2 (/etc/db2tab) March 2004
# --------------------------------------------------------------------------------------------------
# - Version 5 - September 2004
# 05.00a  - Adapted to run under Linux
# --------------------------------------------------------------------------------------------------
# 05.00b  - Verification of new HDLM path Status
# --------------------------------------------------------------------------------------------------
# 05.00c  - Check Second Instance of TSM (Novell) for database and Databse log Percentage used
#         - Unless the file $SLAMDIR/cfg/use_scp.txt is present SLAM will use rcp to copy file to 
#           SLAM server. If file exist it will use scp
#         - Add logic to check TSM for missed schedule
#         - ps command is issue once and output to a file - The file is used during program 
#           execution
# --------------------------------------------------------------------------------------------------
# 05.00d  - Touch problem on linux
# --------------------------------------------------------------------------------------------------
# 05.00e  - Missed schedule was not checking 2nd TSM Instance (Novell/Exchange)
# --------------------------------------------------------------------------------------------------
# 05.00f  - Missed schedule paged 18 Hours after - Added eddate=today in query event command
# --------------------------------------------------------------------------------------------------
# 05.00g  - Added check of hplog on linux for harware errors (fans, temperature, memory, power, disk
#           M.Ford 21 mars 2005
# --------------------------------------------------------------------------------------------------
# 05.00h  - Added check for HDLM on Linux 
# --------------------------------------------------------------------------------------------------
# 05.00i  - Modified for Linux - scp Openssh does not work when server is using ssh.com (use sftp)
# --------------------------------------------------------------------------------------------------
# 05.00j  - Modified for Linux - New Oracle instance was detected with the last letter missing
# --------------------------------------------------------------------------------------------------
# 05.00k  - Was not check TSM Novell for missed schedule
# --------------------------------------------------------------------------------------------------
# 05.00l  - ps -ef Under Linux is only 80 characters long ???? - fix in this version (15 March 2006)
# --------------------------------------------------------------------------------------------------
# 05.00m  - Message when running a script - Number of minutes was not appearing (15 March 2006)
# --------------------------------------------------------------------------------------------------
# 05.00n  - Add Logic to test websphere applications
# --------------------------------------------------------------------------------------------------
# 05.00o  - Add Logic to test HTTP Server (IBM EPlanet)
# --------------------------------------------------------------------------------------------------
# 05.00p  - Decembre 2006 
#       Add export COLUMNS=4096 before doing ps for websphere monitoring
#           Corrected problem when Websphere instance name beginning with same name as another one
# --------------------------------------------------------------------------------------------------
# 05.00q  - Janvier 2007
#           change linux ps -ef to ps -efwww for getting all info on command line
# --------------------------------------------------------------------------------------------------
# 05.00r  - Fevrier 2007
#            change test si backup est fait pour DB2 pour le sommeil de Roch
#           change while (glob("$ADSMLOG_DIR/rc\.$HOSTNAME\.*$winstance*\.log"))
#           pour   while (glob("$ADSMLOG_DIR/rc\.*$HOSTNAME\.*$winstance*\.log"))
#           
# --------------------------------------------------------------------------------------------------
# 06.1  - August 2008 
#          WebSphere instance modification to remove jython/python script running occasionally
#           
# --------------------------------------------------------------------------------------------------
# 06.2  - January 2009  
#          Added - Automatic Cluster detection on Linux
#          Added - Check Cluster Service Status (Should be Started)
#          Added - Check Cluster Daemon - Check if they are running - Added in config file
# --------------------------------------------------------------------------------------------------
# 06.3  - January 2009 
#         CPU Usage was wrong on Linux 
# --------------------------------------------------------------------------------------------------
# 06.4  - January 2009 
#         WebSphere verification 
# --------------------------------------------------------------------------------------------------
# 06.5  - January 2009 
#         Load Average were using 15min values instead of 5 minutes value in Linux and AIX
# --------------------------------------------------------------------------------------------------
# 06.6  - March 2009 
#         Remove HDLM Datapath from slam (were used for old version)
# --------------------------------------------------------------------------------------------------
# 06.7 - Janvier 2010 - Check /etc/db2tab to see if Database are not accessible 
#        Call DBA script every 5 min. db_test_connect_master.sh 
#        Remove heck for db2loggr
#
# --------------------------------------------------------------------------------------------------
# 7.1 -  January 2011 - Changed to feed SCOM moitor instead of SLAM server
#                     - Include automated filesystem increase
# --------------------------------------------------------------------------------------------------
# 7.5    August 2011 - Minor Corrections - daemon checking & WebSphere process checking
#
# --------------------------------------------------------------------------------------------------
# 7.6    September 2011  - Minor Corrections and merge was monitor error with was.txt 
# --------------------------------------------------------------------------------------------------
# 7.7    September 2011  - Reset Date & Time when not monitoring is in effect (ex. cpu usage)
#
# --------------------------------------------------------------------------------------------------
# 7.8    September 2011  - Change the way WebSphere App.Server are detected and monitored
# 7.9    September 2011  - Solve problem with server using Tectia SSH V5
#                        - Correct Problem with Was AppServer Checking
#                        - Correct Problem with Checking IBM HTTP Server instance
# 8.0    October 2011    - Does only monitor Oracle on Certain servers (Eliminate oracle_ in cfg)
#                        - Eliminate double error on SUN HTTP
#                        - Send Email when a scp or rcp failed to SCOM or Sysinfo Server
# --------------------------------------------------------------------------------------------------
# 8.1    Octobre 2012    - Modification pour support de SSH2 Tectia V6
#                        - Nouvelle version de TSM 6.3
# --------------------------------------------------------------------------------------------------
# --------------------------------------------------------------------------------------------------
# 8.2    July 2013       - Correct Some SSH Usage with Rsync (Tectia and OpenSSH Difference & Version)
# --------------------------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------------------------
my $VERSION_NUMBER = "8.3";                                 # xSCOM Version Number
my @slam_array = ();                                        # Array That contain slam.cfg file
my %df_array   = ();                                        # Array Containing Actual Filesystem info
my $OSNAME     = `uname -s`; chomp $OSNAME;                 # Get Name of Operating System 
$OSNAME        =~ tr/A-Z/a-z/;                              # Make Sure OS name is in lowercase (linux aix)
#print "OSNAME  = $OSNAME\n" ;                               # Print OS Name should be aix or linux
my $HOSTNAME   = `hostname -s`; chomp $HOSTNAME;            # Host name of current Machine
my $DEBUG      = "$ENV{'SLAM_DEBUG'}" || "5";               # Set for debugging purpose set to 5 or 6-7
my $DSM_DIR    = "$ENV{'DSM_DIR'}" || "/opt/tivoli/tsm/client/ba/bin64"; # TSM Client Executable Dir.
my $SLAMDIR="/scom";                                        # Directory where slam file reside
my $SLAMBIN_DIR  = "$SLAMDIR/bin";                          # Slam bin Directory
my $SLAMTMP_DIR  = "$SLAMDIR/tmp";                          # Slam temp Directory
my $SLAMLOG_DIR  = "$SLAMDIR/log";                          # Slam LOG Directory
my $SLAMRPT_DIR  = "$SLAMDIR/rpt";                          # Slam result files 
my $SLAMCFG_DIR  = "$SLAMDIR/cfg";                          # Slam Configuration Directory
my $ADSMLOG_DIR = "/var/adsmlog";                           # Where we create the ADSM & Backtrack Backup Log
my $SLAMSCR_DIR  = "$SLAMDIR/scripts";                      # Slam Script Directory
my $SLAMSCR_DBA  = "$SLAMSCR_DIR/db_test_connect_master.sh";# Slam DB2 SCRIPT
my $SLAMTXT_DIR  = "$SLAMDIR/txt";                          # SCOM Result text file directory
...
my $SLAMTMP_FILE = "$SLAMDIR/tmp/$HOSTNAME.tmp";            # Slam temp work file
...
my $SLAMLCK_FILE = "$SLAMDIR/scom.lock";                    # Slam Lock file
...
my $TSMTMP  = "$SLAMDIR/tmp/tsmsched.tmp";                  # Output of TSM command will in this file
my $TSMTMP1 = "$SLAMDIR/tmp/tsmsched1.tmp";                 # Output of TSM command will in this file
my $TSMTMP2 = "$SLAMDIR/tmp/tsmsched2.tmp";                 # Output of TSM command will in this file
#
# HTTP match 
...
# SCOM and xSCOM Related Variables
my $SCOM_DSERVER = "nmmq1d24.slacdev.ca" ;                  # Scom Dev Server - Host Name 
my $SCOM_DUSER   = "dsccsrv";                               # SCOM Dev. UserName
my $SCOM_PSERVER = "nmmq1p53.slac.ca" ;                     # Scom Prod. Server - Host Name 
...
my $SCOM_CFG_FILE = "${SLAMCFG_DIR}/xscom.cfg";             # xSCOM Configuration File
...
my $XSCOM_ADMIN_EMAIL= "aixteam\@standardlife.ca";   # Unix Admin Email Address
...
# ==================================================================================================
# FUNCTION TO LOAD THE CONTENT OF $SLAMDIR/CFG/`HOSTNAME`.CFG FILE IN AN ARRAY CALLED @SLAM_ARRAY.
# ==================================================================================================
sub load_slam_file {


# Establish If we need to monitor Oracle Instance on this server
# Set Variable $DO_NOT_MONITOR_ORACLE to 1 to not monitor and 0 to monitor Oracle Instance
   if (($HOSTNAME eq "sxmq1013") || ($HOSTNAME eq "sxmq1014") || ($HOSTNAME eq "sxmq1003") ||
       ($HOSTNAME eq "sxmq1008") || ($HOSTNAME eq "f2n13")    || ($HOSTNAME eq "sxmq1018")) {
      $DO_NOT_MONITOR_ORACLE=0;   
   }else{
#      if ($SERVER_TYPE eq "D") {
         $DO_NOT_MONITOR_ORACLE=1;   
#      }else{
#         $DO_NOT_MONITOR_ORACLE=0;   
#      }
   }
...
# For debug purpose
   if ($DEBUG >= 5) {
      ...
      print "SCOMDIR                  = ${SLAMDIR}\n";      
      print "Hostname                 = ${HOSTNAME}\n" ;
      print "Virtual Server           = ${VM}\n" ;
      print "In a Cluster             = ${CLUSTER}\n" ;
      print "TSM Exclude File         = ${DSM_INCL_FILE}\n";
      print "Server Type              = ${SERVER_TYPE}\n";
      print "Make Remote Dir          = ${MKDIR_REMOTE}\n";
      print "TSM Client bin Directory = ${DSM_DIR}\n";
      print "Mon. Oracle(0=yes,1=no)  = ${DO_NOT_MONITOR_ORACLE}\n";
      print "SSH_BIN                  = ${SSH_BIN}\n";
      if ( $OSNAME eq "aix" ) { print "Tectia SSH Version is    = $SSH_VER\n";}
      if ( $DO_NOT_MONITOR_ORACLE == 1 ) {
         print "Monitor Oracle on Server = No\n";
      }else{
         print "Monitor Oracle on Server = Yes\n"
      }
      print "------------------------------------------------------------------------------\n";
   }
...
# Open slam server.cfg file and load it in an array called slam_array
# ==================================================================================================
   ...
   while ($line = <SLAMFILE>) {                             # Read while end of file
         ...
         $slam_array[$widx++] = $line ;                     # It will be rewritten at the end
...
# --------------------------------------------------------------------------------------------------
   if ($DEBUG >= 5) {                                       # If in debug mode 
      $nbline = @slam_array;                                # Get number of element loaded
      print "File $SLAMCFG_FILE loaded in slam_array ($nbline lines loaded)\n";
   }
   if ($DEBUG >= 6)  { display_slam_array ; }
}
...
# --------------------------------------------------------------------------------------------------
   ...
   if ($DEBUG >= 6) {       
      print "\n-----\nI am unloading the array \"slam_array\" to the file $SLAMCFG_FILE\n" ;
   }

# Unload slam_array to Disk
# --------------------------------------------------------------------------------------------------
   for ($widx = 0; $widx < @slam_array; $widx++) {             # Loop until end of array
      print (SLAMTMP "$slam_array[$widx]");                    # Write line to config file
   }
...
# FUNCTION CALLED TO DISPLAY SLAM_ARRAY CONTENT (FOR DEBUG PURPOSE)
# --------------------------------------------------------------------------------------------------
sub display_slam_array {
   for ($widx = 0; $widx < @slam_array; $widx++) { 
      print ("$slam_array[$widx]"); 
   }
}
...
sub filesystem_increase {
   ...
   my $FS_SCRIPT = "${SLAMBIN_DIR}/$SLAM_RECORD->{SLAM_SCRIPT}";
...
# From the slam_array extract the application name
...
# From the slam_array extract the daemon name
   @dummy = split /_/, $SLAM_RECORD->{SLAM_ID} ;
   $daemon_name = $dummy[1];
...
# From the slam_array extract the instance name
   @dummy = split /_/, $SLAM_RECORD->{SLAM_ID} ;
   $winstance = $dummy[2];
...
# Get all Backup log files and isolate line with return code 1 or 2
   while (glob("$ADSMLOG_DIR/rc\.$HOSTNAME\.*$winstance*.log")) {
         system ("tail -1 $_ >> $SLAMDIR/tmp/$winstance.$$");
   }


# Check the ADSM log file to see if a database backup is running
   if ( -e "$SLAMDIR/tmp/$winstance.$$"  ) {
      open (ADSMLOG, "cat $SLAMDIR/tmp/$winstance.$$ |") or die "Cannnot open $SLAMDIR/tmp/$winstance.$$: $!\n";
...
# I did not tranpose the script in Perl cause script are different from prod. to dev. machine
   if ( $OSNAME eq "linux" ) {
      $return_code = system ("$SLAMDIR/linux/mqcheck");
   }else{
...
# CHECK FOR NEW FILESYSTEM - IF THEY ARE NOT IN SLAM_ARRAY THEN INSERT THEM 
# --------------------------------------------------------------------------------------------------
sub check_for_new_filesystems  {
   ...
   while ($filesys = <DF_FILE>) {
      ...
      # Try to locate the filesystem in SLAM Array 
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if ($slam_array[$index] !~ /^FS/ && ($slam_array[$index] !~ /^aix_fs/ )) ;
         split_fields($slam_array[$index]);
      ...
      # If filesystem not in slam_array then Insert new filesystem in slam.array
      if ($found eq "N" ) {
         ...
         $index=@slam_array;
         $slam_array[$index] = combine_fields() ;
         }
   }
   close DF_FILE;
}
...
# Process all db2loggr process found
   while ( $oline = <DB2DB> ) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {
         next if $slam_array[$index] !~ /^db2db_${db}_db2loggr/ ;
         $found="Y";
         last;
      }
...
# Read Owner (Instance name) of every db2sysc running
   while ($oline = <DB2FILE>) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {
         next if $slam_array[$index] !~ /^db2i_${instance}_db2sysc/ ;
         $found="Y";
         last;
      }
...
# Read everyline where ora_bwr in running
   while ($oline = <DF_FILE>) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if $slam_array[$index] !~ /^oracle_instance_${instance}/ ;
         $found="Y";
         last;
      }
...
# Read evry WebPshere Application name that are running
   while ($oline = <WEB_FILE>) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if $slam_array[$index] !~ /^websphere_${webapp} / ;
         $found="Y";
         last;
      }
...
# Read the result one line at a time.
   while ($cline = <CLUSERV>) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if $slam_array[$index] !~ /^cserv-${SERVICE}/ ;
...
# Read everyline that match
   while ($oline = <WEB_FILE>) {
      ...
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if $slam_array[$index] !~ /^http-${HTTP}/ ;
         $found="Y";
         last;
      }
...
# Try to locate the HTPP in slam array
      $found="N";
      for ($index = 0; $index < @slam_array; $index++) {     
         next if $slam_array[$index] !~ /^sun-http_${HTTP}/ ;
         $found="Y";
         last;
      }
...
sub check_cluster_services {
   # From the slam_array extract the service name
   @dummy = split /-/, $SLAM_RECORD->{SLAM_ID} ;
...
# IF THE NUMBER OF SECONDS BETWEEN NOW AND THE LAST RUN TIME IS GREATER THAN WANTED - ISSUE ERROR
   if ( $elapse_second >= $SCRIPT_MIN_SEC_BETWEEN_EXEC ) {              # Elapsed Sec >= 86400 Sec. then ok to run
      ...
      my $mail_message2 = "xSCOM Automatically executed restart script : $SLAM_RECORD->{SLAM_SCRIPT}";
      ...
      my $mail_subject = "xSCOM Notification: $HOSTNAME daemon $daemon_name restarted";
...
sub email_error {
   ...
   my $MAIL_SUBJECT = "xSCOM File transfer problem on $HOSTNAME";
   #$COMMAND =  "echo \"$MAIL_MESSAGE\" | $CMD_MAIL -s \"$MAIL_SUBJECT\" $XSCOM_ADMIN_EMAIL";
#   @cmd =      "echo \"$MAIL_MESSAGE\" | $CMD_MAIL -s \"$MAIL_SUBJECT\" $XSCOM_ADMIN_EMAIL";
...
sub copyfile2scomserver {
   ...
   if ( ${MKDIR_REMOTE} eq "Y" ) {
      ...
      print "\nIf you want to create them, modify $SCOM_CFG_FILE (mkdir_remote = Y) & rerun xSCOM";
      
   }


   # COPY TXT FILES TO SCOM SERVER
   #  scp -rP 32  /scom/txt/*.txt dsccsrv@nmmq1d24.slacdev.ca:\lxmq0007
   # -----------------------------------------------------------------------------------------------
...
# THIS FUNCTION IS CALLED AT THE BEGINNING TO CREATE A LOCK FILE IN $SLAMDIR/SLAM.LOCK
...
# UNLOAD THE UPDATED VERSION OF SLAM_ARRAY TO SLAM.CFG FILE
# --------------------------------------------------------------------------------------------------
sub loop_through_array {

# Loop Through All Slam.cfg File in Memory
   for ($index = 0; $index < @slam_array; $index++)                     # Treat each line one at a time
      {  
      next if $slam_array[$index] =~ /^#/ ;                             # Don't process comment line
      next if $slam_array[$index] =~ /^$/ ;                             # Don't process blank line
      split_fields($slam_array[$index]);                                # Split line into fields
...
# IF A START OR AN END TIME WAS SPECIFIED , WE NEED TO VERIFY IF THE LINE IS ACTIVE AT CURRENT TIME
      ...
      if ($SLAM_RECORD->{SLAM_STHRS} != 0 and $SLAM_RECORD->{SLAM_ENDHRS} != 0) {
         ...
         if ($SLAM_RECORD->{SLAM_ENDHRS} < $SLAM_RECORD->{SLAM_STHRS}) {
            if (($current_time > $SLAM_RECORD->{SLAM_STHRS})  || ($current_time < $SLAM_RECORD->{SLAM_ENDHRS})) { 
               ...
               $evaluate_line="no"; 
               $slam_array[$index] = combine_fields(); # Combine all fields & put it back into array 
            }   
         }else{
            if (($current_time >= $SLAM_RECORD->{SLAM_STHRS}) && ($current_time <= $SLAM_RECORD->{SLAM_ENDHRS})) { 
               $evaluate_line="yes"; 
      ...
      $slam_array[$index] = combine_fields() ;              # Combine all the fields into a line and put it back into the array 
   }
}
